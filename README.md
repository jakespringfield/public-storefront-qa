...
- Final responses must parse to exactly `text/html` or `application/xhtml+xml`; MIME parameters are parsed separately. Unsupported or failing declared charsets fall back to UTF-8 replacement, decoded surrogate code points are replaced, and malformed HTML parser input becomes an explicit `UNAVAILABLE` observation instead of crashing the capture.
